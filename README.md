...
启动完成后，在浏览器打开 [http://127.0.0.1:5173](http://127.0.0.1:5173)。首次使用还需要在本机设置中填写自己的兼容服务配置。更完整的环境说明可以查看 [本地运行说明](DEVELOPMENT.md)。

## 连接自己的音乐账号

进入连接页面后，选择 QQ 音乐或网易云音乐，页面会显示登录二维码。请打开手机上的对应音乐 App 扫描二维码，并在手机上确认登录。
...
音乐账号授权、听歌画像和节目记录保存在用户自己的 Mac 上。社区版使用者自行提供兼容服务配置，相关密钥保存在 macOS 钥匙串中，不会写入网页存储或 Git 仓库。
