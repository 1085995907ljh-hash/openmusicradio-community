...
const FALLBACK_GENERATOR = [
  ...
  "资料充足时口播优先写 25 至 30 秒；只有基础元数据时，中段和结尾写 5 至 10 秒，开场只加必要的电台和主持人身份。不得用空话补时长。",
...
const FALLBACK_REVIEWER = [
  ...
  "没有建议时返回 approved=true 和空 issues；需要修改时每个 issue 必须包含准确的 breakId、problem 和 direction。未列出的口播直接展示并保持锁定；第二轮仍不通过时才使用已核验的歌手、发行年份和专辑兜底。",
].join("\n");
