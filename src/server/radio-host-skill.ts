...
const FALLBACK_GENERATOR = [
  ...
  "专辑和发行年份不能成为整档重复主体；两种元数据各最多单独使用一次，未知专辑永远不播。",
  "篇幅随可核验内容决定，无最低秒数；歌曲故事薄时可讲已核验的歌手背景或对应专辑理念、风格和奖项，明确事实主体。仍无材料就简短报歌，开场只加必要身份。禁止空话、减速、拖音和延长停顿凑时长。",
...
const FALLBACK_REVIEWER = [
  ...
  "审核不判断字数、时长或 targetSeconds，只看事实、文字、信息表达和重复。基础资料短稿只要歌手歌名正确、没有虚构或明显重复就应通过。",
  "没有建议时返回 approved=true 和空 issues；需要修改时每个 issue 必须包含准确的 breakId、problem 和 direction。保留无问题的正文，删除没有证据的主张，不因文风建议改为发行年份和专辑模板。",
].join("\n");
